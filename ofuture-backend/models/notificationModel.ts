...
class NotificationModel {
  ...
  async getByUserId(
    ...
    const conn = await pool.getConnection();
    try {
      const [rows] = await conn.execute(query, [userId, limit, offset]) as unknown as [NotificationData[], unknown];
      return rows;
    } finally {
      conn.release();
    }
  ...
  async getUnreadCount(userId: string): Promise<number> {
    ...
    const conn = await pool.getConnection();
    try {
      const [rows] = await conn.execute(query, [userId]) as unknown as [RowDataPacket[], unknown];
  ...
  async getById(id: string): Promise<NotificationData | null> {
    ...
    const conn = await pool.getConnection();
    try {
      const [rows] = await conn.execute(query, [id]) as unknown as [NotificationData[], unknown];
      return rows[0] || null;
    } finally {
      conn.release();
    }
  ...
  async getRecentUnread(userId: string, limit: number = 5): Promise<NotificationData[]> {
    ...
    const conn = await pool.getConnection();
    try {
      const [rows] = await conn.execute(query, [userId, limit]) as unknown as [NotificationData[], unknown];
      return rows;
    } finally {
      conn.release();
    }
