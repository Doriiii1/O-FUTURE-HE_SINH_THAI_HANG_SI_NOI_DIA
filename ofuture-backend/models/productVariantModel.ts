...
class ProductVariantModel {
  ...
  static async getById(id: string): Promise<ProductVariant | null> {
    ...
    try {
      ...
      const [rows] = await conn.execute(query, [id]) as unknown as [ProductVariant[], unknown];
  ...
  static async getBySku(sku: string): Promise<ProductVariant | null> {
    ...
    try {
      ...
      const [rows] = await conn.execute(query, [sku]) as unknown as [ProductVariant[], unknown];
      return rows.length > 0 ? rows[0] : null;
    } finally {
      conn.release();
    }
  ...
  static async getByProductId(productId: string): Promise<ProductVariant[]> {
    ...
    try {
      ...
      `;
      const [rows] = await conn.execute(query, [productId]) as unknown as [ProductVariant[], unknown];
      return rows;
    } finally {
      conn.release();
    }
  ...
  static async getGroupedByProduct(productId: string): Promise<VariantGroup[]> {
    ...
    try {
      const query = `
        ...
        ORDER BY attribute_name ASC, attribute_value ASC
      `;
      const [variants] = await conn.execute(query, [productId]) as unknown as [ProductVariant[], unknown];
