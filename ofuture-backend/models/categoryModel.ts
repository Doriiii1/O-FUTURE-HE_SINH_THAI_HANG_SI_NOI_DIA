...
class CategoryModel {
  ...
  static async getById(id: string): Promise<Category | null> {
    ...
    try {
      ...
      const [rows] = await conn.execute(query, [id]) as unknown as [Category[], unknown];
  ...
  static async getBySlug(slug: string): Promise<Category | null> {
    ...
    try {
      ...
      const [rows] = await conn.execute(query, [slug]) as unknown as [Category[], unknown];
      return rows.length > 0 ? rows[0] : null;
    } finally {
      conn.release();
    }
  ...
  static async getTopLevel(): Promise<Category[]> {
    ...
    try {
      ...
      `;
      const [rows] = await conn.execute(query) as unknown as [Category[], unknown];
      return rows;
    } finally {
      conn.release();
    }
  ...
  static async getByParentId(parentId: string): Promise<Category[]> {
    ...
    try {
      const query = `
        ...
        ORDER BY display_order ASC, name ASC
      `;
      const [rows] = await conn.execute(query, [parentId]) as unknown as [Category[], unknown];
      return rows;
    } finally {
      conn.release();
    }
