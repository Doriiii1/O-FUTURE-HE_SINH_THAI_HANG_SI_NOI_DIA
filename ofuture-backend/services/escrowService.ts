...
const openDispute = async ({ orderId, buyerId, reason, ipAddress }: OpenDisputeParams) => {
  ...
  try {
    ...
    if (escrow.status === 'disputed') {
      // Idempotent behavior: if a dispute already exists, return success and
      // the existing disputeId instead of failing with 409.
      await conn.execute(
        `INSERT INTO disputes
         ...
         ON DUPLICATE KEY UPDATE
           ...
           status = 'pending'`,
        [escrow.id, orderId, buyerId, reason]
      );
      ...
      return {
        success: true,
        message: 'Đơn hàng này đã có khiếu nại rồi.',
        orderId,
    ...
    // Also create a record in `disputes` so buyer/seller pages can render it.
    // Use escrow_transaction id as disputes.id to keep compatibility with admin resolution payload.
    await conn.execute(
      `INSERT INTO disputes
         (id, order_id, complainant_id, reason, evidence_urls, status)
       VALUES (?, ?, ?, ?, NULL, 'pending')
...
const resolveDispute = async ({ orderId, disputeId, adminId, resolution, reason, ipAddress }: ResolveDisputeParams) => {
  ...
  try {
    ...
    // Some admin flows may not provide disputeId. Fall back to escrow_transaction id,
    // which we store as disputes.id when dispute is created via escrow/dispute.
    const effectiveDisputeId = disputeId || escrow.id;
