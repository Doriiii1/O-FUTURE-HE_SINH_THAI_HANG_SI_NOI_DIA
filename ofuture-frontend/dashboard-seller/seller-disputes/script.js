...
function renderDisputes() {
  ...
  if (currentStatus !== 'all') {
    // Backend enum: pending | resolved_released | resolved_refunded | rejected
    // UI tabs (legacy): open | in_progress | resolved | closed
    if (currentStatus === 'open' || currentStatus === 'in_progress') {
      filtered = allDisputes.filter(d => d.status === 'pending');
    } else if (currentStatus === 'resolved') {
      filtered = allDisputes.filter(d => d.status === 'resolved_released' || d.status === 'resolved_refunded');
    } else if (currentStatus === 'closed') {
      filtered = allDisputes.filter(d => d.status === 'rejected');
    } else {
