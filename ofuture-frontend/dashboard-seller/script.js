...
const CATEGORY_OPTION_PRESETS = {
    Fashion: [
        ...
        { key: 'color', label: 'Màu Sắc (Đỏ, Đen...)' },
        { key: 'material', label: 'Chất Liệu (Cotton, Denim...)' }
    ],
    Electronics: [
        { key: 'model', label: 'Cấu Hình (Pro, Plus...)' },
        { key: 'storage', label: 'Dung Lượng (128GB, 256GB...)' },
        { key: 'color', label: 'Màu Sắc (Đỏ, Đen...)' }
    ],
    Beauty: [
        { key: 'volume', label: 'Dung Tích (30ml, 100ml...)' },
        { key: 'skin_type', label: 'Loại Da (Da khô, Da dầu...)' },
        ...
        <tr>
            ...
            <td>
                <div style="display:flex; gap:8px; flex-wrap:wrap;">
                    <a class="btn btn-small btn-primary" style="text-decoration:none;" href="../dashboard-buyer/buyer-dispute-detail/index.html?id=${d.id}">Xem trò chuyện</a>
                    ${d.status === 'pending' 
...
window.handleSample = async function(sampleId, status) {
    ...
    // Backend chỉ chấp nhận: approved, shipped, cancelled, returned
    // Map trạng thái "rejected" trên UI về "cancelled" trong DB để không vỡ luồng xử lý.
