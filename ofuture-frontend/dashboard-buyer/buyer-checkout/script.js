...
function loadCheckoutData() {
    ...
    if (pendingOrderId) {
        ...
        // Backfill breakdown (tạm tính / phí nền tảng) khi thanh toán lại
        // Theo logic đơn mới: platformFee = subtotal * 0.025, shippingFee = 30000
        ...
        if (total > shippingFee) {
            subtotal = Math.round((total - shippingFee) / 1.025);
            platformFee = Math.round(subtotal * 0.025);
            // Fix lệch làm tròn để tổng khớp đúng với amount truyền vào
            platformFee = Math.max(0, total - subtotal - shippingFee);
        }
...
window.handlePlaceOrder = async function() {
    ...
    try {
        if (pendingOrderId) {
            // Thanh toán lại đơn đang pending từ trang lịch sử
            // Không cần lấy lại giỏ hàng/địa chỉ, dùng luôn pendingOrderId + finalTotalAmount
            if (paymentMethod === 'momo') {
                await processMoMo(pendingOrderId, finalTotalAmount);
            } else if (paymentMethod === 'qr') {
                await processVietQR(pendingOrderId, finalTotalAmount);
                // processVietQR chỉ mở modal, không redirect; bật lại nút cho user thao tác tiếp
                btn.disabled = false;
                btn.textContent = "Xác nhận Đặt hàng";
            } else if (paymentMethod === 'wallet') {
                await processWalletPayment(pendingOrderId, finalTotalAmount);
            }
            return;
        }
