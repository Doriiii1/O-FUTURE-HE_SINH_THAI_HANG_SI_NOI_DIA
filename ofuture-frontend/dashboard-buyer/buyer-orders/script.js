...
let currentOrderType = 'orders'; // 'orders' (đơn sỉ) hoặc 'samples' (hàng mẫu)
...
function renderOrders() {
    ...
    container.innerHTML = filtered.map(order => {
        ...
        const effectiveStatus = (order.escrowStatus || order.escrow_status) === 'disputed' ? 'disputed' : order.status;
        const badgeInfo = getStatusBadge(effectiveStatus);
            ...
            <div class="card order-card">
                ...
                <div class="order-body">
                    ...
                    <div style="text-align: right; display: flex; flex-direction: column; justify-content: center;">
                        <p style="font-size: 12px; color: #94a3b8; margin-bottom: 4px;">Trạng thái</p>
...
function getStatusBadge(status) {
    const map = {
        'pending': { text: 'Chờ thanh toán', class: 'badge-pending' },
        'paid': { text: 'Đã thanh toán', class: 'badge-paid' },
        'shipped': { text: 'Đang giao hàng', class: 'badge-shipped' },
        'disputed': { text: 'Đang khiếu nại', class: 'badge-cancelled' },
...
function generateActionButtons(order) {
    ...
    const disputeId = order.disputeId || order.dispute_id || '';
    // ĐƠN HÀNG MẪU (sample_requests) hiện chưa có trang chi tiết riêng
    // → tránh gọi API /orders/:id gây lỗi "Order not found"
    if (currentOrderType === 'samples') {
        // Có thể mở rộng sau: chuyển sang trang chi tiết Yêu cầu Hàng mẫu
        return ''; 
    }

    // ĐƠN SỈ (orders): luôn hiển thị nút "Xem chi tiết"
    ...
    if (escrowStatus === 'disputed') {
        if (disputeId) {
            btns += `<button class="btn btn-outline" style="color: #ef4444; border-color: #ef4444;" onclick="window.location.href='../buyer-dispute-detail/index.html?id=${disputeId}'">⚠️ Xem khiếu nại</button>`;
        }
        return btns;
    }
    ...
    if (order.status === 'pending') {
        ...
        btns += `<button type="button" class="btn btn-outline" style="color:#64748b;border-color:#94a3b8;" onclick="cancelPendingOrder('${id}')">Hủy đơn</button>`;
    }
...
// Hủy đơn chờ thanh toán (đóng MoMo / không muốn mua nữa)
window.cancelPendingOrder = async function(orderId) {
    if (!orderId) return;
    if (!confirm('Hủy đơn hàng này? Bạn có thể đặt lại sau. Đơn ở trạng thái chờ thanh toán sẽ được hủy và tồn kho được hoàn lại.')) return;
    try {
        const response = await fetch(`${API_BASE_URL}/orders/${orderId}/cancel`, {
            ...
            body: JSON.stringify({ reason: 'Người mua hủy trước khi thanh toán' })
        });
        const data = await response.json();
        if (response.ok && data.success) {
            ...
            throw new Error(data.message || 'Không thể hủy đơn.');
        }
    } catch (e) {
        showToast(e.message || 'Lỗi hủy đơn', true);
    }
};
...
window.submitDispute = async function() {
    ...
    try {
        ...
        if (response.ok && data.success) {
            const disputeId = data?.data?.disputeId;
            showToast("Đã gửi khiếu nại thành công! Admin sẽ xem xét.");
            closeDisputeModal();
            fetchOrders();
            if (disputeId) {
                window.location.href = `../buyer-dispute-detail/index.html?id=${disputeId}`;
            }
        } else {
            throw new Error(data.message || "Không thể gửi khiếu nại.");
        }
    } catch (error) {
        // If the dispute already exists, redirect the buyer to that dispute instead of failing.
        if ((error.message || '').includes('held status') || (error.message || '').includes('đã có khiếu nại')) {
            try {
                const existing = await fetchAPI('/disputes/my');
                const match = (existing.data || []).find(d => d.order_id === currentOrderIdForDispute);
                if (match?.id) {
                    closeDisputeModal();
                    showToast('Đơn này đã có khiếu nại. Đang mở chi tiết...', false);
                    window.location.href = `../buyer-dispute-detail/index.html?id=${match.id}`;
                    return;
                }
            } catch (lookupErr) {
                console.warn('Failed to lookup existing dispute:', lookupErr);
            }
        }
        showToast(error.message, true);
    } finally {
