...
function renderProducts(products) {
    ...
    grid.innerHTML = products.map(p => {
        ...
        const isOutOfStock = (p.stockQuantity ?? p.stock_quantity ?? 0) <= 0;
        const stockLeft = p.stockQuantity ?? p.stock_quantity ?? 0;
        ...
        return `
            <div class="product-card">
                ...
                <div class="product-info">
                    ...
                    <div class="product-seller">Cung cấp bởi: <strong>${p.sellerUsername || 'Nhà cung cấp'}</strong></div>
                    <div class="product-price">${price} đ</div>
                        <small class="muted">Tồn kho còn lại: ${stockLeft}</small>
                    
                    <button class="btn btn-primary btn-add-cart" 
                        ${isOutOfStock ? 'disabled' : ''} 
                        onclick='addToCart(${JSON.stringify(String(p.id))}, ${JSON.stringify(String(p.name))}, ${Number(p.price)}, ${JSON.stringify(imgUrl)}, ${JSON.stringify(String(p.sellerUsername || ''))}, ${Number(p.stockQuantity ?? p.stock_quantity ?? 0)})'>
...
window.addToCart = function(id, name, price, image, sellerUsername, stock) {
    if (stock <= 0) return;
    ...
    if (existingItem) {
        ...
        cart.push({ id, name, price, image, sellerUsername, stock, quantity: 1 });
    }
