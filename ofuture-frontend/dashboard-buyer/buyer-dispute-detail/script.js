...
function renderEvidenceGallery() {
  ...
  gallery.innerHTML = evidenceUrls.map((url, idx) => {
    const normalizedUrl = getMediaDetectionUrl(url);
    const isImage = /\.(jpg|jpeg|png|gif|webp)$/i.test(normalizedUrl);
    const isVideo = /\.(mp4|webm|mov)$/i.test(normalizedUrl);
...
function renderDisputeTimeline() {
  ...
  if (currentDispute.resolved_at) {
    events.push({
      ...
      text: `Tranh chấp đã được giải quyết: ${getStatusLabel(currentDispute.status)}`
    });
  }
...
function renderChatMessage(message) {
  ...
  if (message.attachments && message.attachments.length > 0) {
    const attachmentItems = message.attachments.map(url => {
      const detectionUrl = getMediaDetectionUrl(url);
      const isImage = /^data:image\//i.test(url) || /\.(jpg|jpeg|png|gif|webp)$/i.test(detectionUrl);
      const isVideo = /^data:video\//i.test(url) || /\.(mp4|webm|mov)$/i.test(detectionUrl);
      const filename = detectionUrl.split('/').pop() || 'tep-dinh-kem';

      if (isImage) {
        return `<a href="${url}" target="_blank" class="attachment-thumb"><img src="${url}" alt="${filename}"></a>`;
      }

      if (isVideo) {
        return `<a href="${url}" target="_blank" class="attachment-thumb"><video muted playsinline><source src="${url}"></video></a>`;
      }
...
function setupEventListeners(disputeId) {
  ...
  document.getElementById('fileInput').addEventListener('change', (e) => {
    ...
    handleSelectedAttachments(files);
  });
...
async function handleSelectedAttachments(files) {
  const acceptedFiles = files.filter(file => {
    const isImage = file.type.startsWith('image/');
    const isVideo = file.type.startsWith('video/');
    const isSmallEnough = file.size <= 5 * 1024 * 1024;
    return (isImage || isVideo) && isSmallEnough;
  });

  if (acceptedFiles.length === 0) {
    showToast('Chỉ hỗ trợ ảnh/video dưới 5MB.', 'error');
    return;
  }

  try {
    const encodedFiles = await Promise.all(acceptedFiles.map(readFileAsDataUrl));
    attachedUrls.push(...encodedFiles);
    renderAttachmentPreview();
    showToast('Đã đính kèm tệp thành công.', 'success');
  } catch (error) {
    console.error('Attachment read error:', error);
    showToast('Không thể đọc tệp đính kèm.', 'error');
  }
}
...
function renderAttachmentPreview() {
  ...
  preview.innerHTML = attachedUrls.map((url, idx) => {
    ...
    if (isImage) {
      return `
        <div class="attachment-preview-item media">
...
function getMediaDetectionUrl(url) {
...
function getStatusLabel(status) {
  const labels = {
    ...
    'resolved_released': 'Đã giải ngân',
    'resolved_refunded': 'Đã hoàn tiền',
    'rejected': 'Bị từ chối',

    // legacy labels (older dispute UI)
    'open': 'Mở',
