...
function renderActionButtons(order, container) {
  ...
  const disputeId = order.disputeId || order.dispute_id || null;
  const escrowStatus = order.escrow?.status || order.escrow_status || null;
  if (escrowStatus === 'disputed') {
    btns += `<a class="btn btn-outline" style="width: 100%; color: #ef4444; border-color: #ef4444; text-decoration: none;" href="../buyer-dispute-detail/index.html?id=${disputeId || id}">⚠️ Xem khiếu nại</a>`;
