            ...
            container.innerHTML = data.data.map(p => `
                <div class="product-item">
                    <div class="product-info">
                        ...
                        <small class="muted">Tồn kho: ${p.stockQuantity}</small>
                    </div>
                    <button class="btn btn-primary" style="padding: 6px 12px; font-size: 13px;" 
                        onclick='addToCart(${JSON.stringify(String(p.id))}, ${JSON.stringify(String(p.name))}, ${Number(p.price)}, ${Number(p.stockQuantity ?? p.stock_quantity ?? 0)}, ${JSON.stringify(String(p.sellerUsername || ''))})'>
                        + Vào giỏ
                    </button>
                </div>
            `).join('');
...
window.addToCart = function(id, name, price, stock, sellerUsername = '') {
    if (stock <= 0) {
    ...
    if (existingItem) {
        ...
        cart.push({ id, name, price, stock, sellerUsername, quantity: 1 });
    }
