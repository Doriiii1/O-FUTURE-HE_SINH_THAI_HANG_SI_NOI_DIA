...
async function loadStats() {
    try {
        ...
        if (document.getElementById('totalEscrowBalance')) document.getElementById('totalEscrowBalance').textContent = formatVND(stats.escrow?.total_held || 0);
        
        if (document.getElementById('escrowHeld')) document.getElementById('escrowHeld').textContent = formatVND(stats.escrow?.total_held || 0);
        if (document.getElementById('escrowReleased')) document.getElementById('escrowReleased').textContent = formatVND(stats.escrow?.released || 0);
        if (document.getElementById('escrowPending')) document.getElementById('escrowPending').textContent = formatVND(stats.escrow?.pending || 0);
...
async function loadSellerProfileRequests() {
    try {
        const res = await apiCall('/admin/seller-profile-change-requests');
        allSellerProfileRequests = res.data || [];
        renderSellerProfileRequests();
    } catch (error) {
        const tbody = document.getElementById('sellerProfileRequestsBody');
        if (tbody) tbody.innerHTML = '<tr><td colspan="5" class="text-center" style="color:red;">Lỗi tải yêu cầu đổi hồ sơ seller</td></tr>';
    }
}
...
function renderSellerProfileRequests() {
    const tbody = document.getElementById('sellerProfileRequestsBody');
    if (!tbody) return;
    if (!allSellerProfileRequests.length) {
        tbody.innerHTML = '<tr><td colspan="5" class="text-center">Chưa có yêu cầu nào</td></tr>';
        return;
    }

    tbody.innerHTML = allSellerProfileRequests.map((r) => {
        let changes = {};
        try {
            changes = typeof r.requested_changes === 'string' ? JSON.parse(r.requested_changes) : (r.requested_changes || {});
        } catch (e) {
            changes = {};
        }
        const changesText = Object.entries(changes)
            ...
            .join('') || '<span class="text-muted">Không có dữ liệu</span>';

        return `
            <tr>
                <td>${r.seller_username || r.seller_full_name || (r.seller_id || '').substring(0, 8)}</td>
                <td>${changesText}</td>
                ...
                <td>
                    ${r.status === 'pending'
                        ? `<button class="btn btn-small btn-success" onclick="reviewSellerProfileRequest('${r.id}','approved')">Duyệt</button>
                           <button class="btn btn-small btn-danger" onclick="reviewSellerProfileRequest('${r.id}','rejected')">Từ chối</button>`
                        : '-'}
                </td>
            </tr>
        `;
...
function renderEscrowTable() {
    ...
    tbody.innerHTML = allEscrow.map(escrow => `
        <tr>
            ...
            <td>${escrow.buyer?.username || escrow.buyer || escrow.buyer_username || 'Unknown'}</td>
            <td>${escrow.seller?.username || escrow.seller || escrow.seller_username || 'Unknown'}</td>
            <td style="color:#2563eb; font-weight:bold;">${formatVND(escrow.amount || 0)}</td>
...
function renderPaymentsTable() {
    ...
    tbody.innerHTML = allPayments.map(payment => `
        <tr>
            ...
            <td>${formatVND(payment.amount || 0)}</td>
...
function renderModerationProducts() {
    ...
    tbody.innerHTML = allModerationProducts.map(p => `
        <tr>
            ...
            <td>${p.name||''}</td>
            <td>${p.seller?.username || p.seller_username || ''}</td>
            <td>${p.category||''}</td>
            <td>${formatVND(Number(p.price||0))}</td>
...
function renderLiveChatsList() {
    ...
    el.innerHTML = allLiveChats.map(c => `
        <li style="list-style:none;margin-bottom:8px; padding:10px; background:#f8fafc; border-radius:8px;">
            <a href="#" onclick="openChat('${c.id}');return false;" style="text-decoration:none; color:#0f172a; font-weight:600;">
                ${c.user?.username||'Guest'} - ${c.id?.substring(0,8)||''}
            </a>
...
function updatePlatformRevenue() {
    ...
    if (!revenueEl) {
        ...
        if (grid) {
            ...
            div.innerHTML = `<h3>Platform Revenue</h3><p id="totalPlatformRevenue">0 đ</p>`;
            grid.appendChild(div); revenueEl = document.getElementById('totalPlatformRevenue');
        }
    }
    if (revenueEl) revenueEl.textContent = formatVND(totalPlatformRevenue);
}
...
window.resolveDispute = async function(escrowId) {
    ...
    if (modal) {
        try {
            ...
            const data = res.data || {};
            document.getElementById('escrowAmountDisplay').textContent = formatVND(data.amount || 0);
...
window.reviewSellerProfileRequest = async function(requestId, decision) {
    let adminNote = '';
    if (decision === 'rejected') {
        adminNote = await showPromptModal('Nhập lý do từ chối:');
        if (adminNote === null) return;
    }
    try {
        await apiCall(`/admin/seller-profile-change-requests/${requestId}`, {
            method: 'PUT',
        ...
        alert('Lỗi xử lý yêu cầu: ' + err.message);
    }
}
...
window.sendChatMessage = async function(chatId, text) {
    try { await apiCall(`/admin/chats/${chatId}/reply`, { method: 'POST', body: JSON.stringify({ message: text }) }); openChat(chatId); } catch (err) { alert('Error: ' + err.message); }
}
...
function formatEvidence(e) { 
    ...
    const renderOne = (x) => {
        const val = typeof x === 'string' ? x : (x?.url || x?.text || '');
        if (!val) return '';
        const isImage = /^data:image\//i.test(val) || /\.(jpg|jpeg|png|gif|webp)$/i.test(String(val).split('?')[0]);
        if (isImage) return `<img src="${val}" style="max-width:100%; border-radius:8px; margin-bottom:8px;">`;
        return `<a href="${val}" target="_blank" rel="noopener">${val}</a>`;
    };
    if (Array.isArray(e)) return e.map(renderOne).join('<br>') || 'Không có';
    return renderOne(e) || 'Không có';
}
...
function setupEventListeners() {
    document.querySelectorAll('.menu-item').forEach(item => {
        item.addEventListener('click', (e) => {
            ...
            if(section === 'users') loadSellerProfileRequests();
